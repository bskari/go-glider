...
func TestParseSentence(t *testing.T) {
	...
	if telemetry.previousPoint.Latitude != 37.0 {
		t.Error("Failed to parse RMC latitude")
	}
	if telemetry.previousPoint.Longitude != -133.0 {
	...
	if telemetry.previousPoint.Latitude != -43.0 {
		t.Error("Failed to parse GGA latitude")
	}
	if telemetry.previousPoint.Longitude != 40.0 {
